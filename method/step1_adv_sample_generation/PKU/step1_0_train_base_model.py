...
from method.step1_adv_sample_generation.PKU.model import BertSeg
from transformers.optimization import get_cosine_schedule_with_warmup, AdamW
from tqdm import tqdm
from method.step1_adv_sample_generation.PKU.metrics import f1_score, bad_case, output_write, output2res
...
def train(train_loader, dev_loader, model, optimizer, scheduler, model_dir, device):
    ...
    for epoch in range(1, epoch_num + 1):
        ...
        if improve_f1 > 1e-5:
            best_val_f1 = val_f1
            final_model_save_path = os.path.join(model_dir, 'best_PKU_bert_base_chinese_.pt')
...
def train_model(dataset_path, model_path):
    word_train, word_dev, label_train, label_dev = load_dataset(dataset_path)

    train_dataset = SegDataset(word_train, label_train)
    dev_dataset = SegDataset(word_dev, label_dev)
    ...
    # dev_loader = DataLoader(dev_dataset, batch_size=batch_size, sampler=DistributedSampler(dev_dataset),
    #                         collate_fn=dev_dataset.collate_fn, num_workers=4)


    bert_model = '/media/usr/external/home/usr/project/project3_data/base_model/bert_base_chinese'
    label2id = {'B': 0, 'M': 1, 'E': 2, 'S': 3}
    model = BertSeg.from_pretrained(bert_model, num_labels=len(label2id))
    ...
    train(train_loader, dev_loader, model, optimizer, scheduler, model_path, device)
