...
class Config:
    ...
    batch_size = 32
    max_length = 256
    output_model_dir = '/media/usr/external/home/usr/project/project3_data/adv_samples/OCNLI/final_adv/roberta_base_chinese'
...
def train(model, train_loader, dev_loader, config):
    ...
    for epoch in range(epochs):
            ...
            if step_cnt % config.eval_steps == 0:
                ...
                if float(dev_accu) > float(best_accu):
                    ...
                    torch.save(model.state_dict(),
                               os.path.join(config.output_model_dir, 'best_OCNLI_roberta_base_chinese.model'))
        ...
        if float(dev_accu) > float(best_accu):
            best_accu = dev_accu
            torch.save(model.state_dict(), os.path.join(config.output_model_dir, 'best_OCNLI_roberta_base_chinese.model'))
            print('new epoch saved as the best model {}'.format(epoch))
        model.train()
