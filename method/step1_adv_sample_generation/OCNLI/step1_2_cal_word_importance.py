...
import os
import pandas as pd
from method.step1_adv_sample_generation.OCNLI.seg_data_to_words import seg_data
...
def gen_new_sentence(data_path):
    ...
    for data_num in range(len(seg_arr)):
        ...
        ori_fact_a = test_data.loc[data_num, 'premise']
        ori_fact_b = test_data.loc[data_num, 'hypothese']
    ...
    merge_dt_dict = {'belong': belong_list, 'belong_text': belong_text, 'str': str_list, 'premise': fact_a_list,
                     'hypothese': fact_b_list, 'label': label_list}
