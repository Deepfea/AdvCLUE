...
from method.step1_adv_sample_generation.OCNLI.load_token_roberta import load_token
...
def get_transferability_mutants(load_path):
    model_path = os.path.join(load_path, 'roberta_base_chinese', 'best_OCNLI_roberta_base_chinese.model')
    ...
    for name_num in range(len(fidelity_mutants_list)):
        ...
        for data_num in range(len(true_labels)):
            ...
            ori_premise_list.append(candidate_mutants.loc[data_num, 'ori_premise'])
            ori_hypothese_list.append(candidate_mutants.loc[data_num, 'ori_hypothese'])
            premise_list.append(candidate_mutants.loc[data_num, 'premise'])
            hypothese_list.append(candidate_mutants.loc[data_num, 'hypothese'])
        ...
        merge_dt_dict = {'belong': belong_list, 'ori_premise': ori_premise_list, 'ori_hypothese': ori_hypothese_list, 'premise': premise_list,
                         'hypothese': hypothese_list, 'str': str_list, 'word': word_list, 'label': label_list, 'type': mut_list}
