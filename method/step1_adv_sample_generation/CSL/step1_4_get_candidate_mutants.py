...
from method.step1_adv_sample_generation.CSL.get_sentence import get_sentence
...
def save_candidate_mutants(test_data, mutant_x, mutant_y, ori_strs, mut_strs, load_rate_path, save_name):
    ...
    for data_num in range(len(test_data)):
        ...
        ori_text_a = test_data.loc[data_num, 'abs']
        ori_text_b = test_data.loc[data_num, 'keyword']
    ...
    merge_dt_dict = {'ori_abs': ori_text_a_list, 'ori_keyword': ori_text_b_list, 'abs': text_a_list, 'keyword': text_b_list,
...
def get_sentences(test_data, select_words, select_word_texts, mutate_list):
    ...
    for data_num in range(len(select_words)):
        text_a = test_data.loc[data_num, 'abs']
        text_b = test_data.loc[data_num, 'keyword']
...
def generate_mutant(data_path, rate):
    ...
    for data_num in tqdm(range(len(test_data))):
        ...
        # print(test_data)
        fact_a = test_data.loc[data_num, 'abs']
        fact_b = test_data.loc[data_num, 'keyword']
