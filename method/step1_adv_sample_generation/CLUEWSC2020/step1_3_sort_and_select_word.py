...
def sort_seg(data_path):
    ...
    seg_positions = np.load(os.path.join(save_path, 'seg_position.npy'), allow_pickle=True)
    sort_segs = []
    sort_scores = []
    sort_positions = []
...
def select_words(data_path, rate):
    ...
    total_seg_num = np.load(os.path.join(data_path, 'words_num_arr.npy'), allow_pickle=True)
    for data_num in range(len(sort_segs)):
        seg_num = total_seg_num[data_num]
        select_seg_num = math.ceil(rate * seg_num)
        select_words = sort_segs[data_num][:select_seg_num]
        # print(select_words)
        select_positions = sort_positions[data_num][:select_seg_num]
        words_list.append(select_words)
        position_list.append(select_positions)
    ...
    print(words_list)
    np.save(os.path.join(save_rate_path, 'select_word_positions.npy'), position_list)
