...
from method.step1_adv_sample_generation.LCQMC.load_token_roberta import load_token
...
def get_transferability_mutants(load_path):
    model_path = os.path.join(load_path, 'roberta_base_chinese', 'best_LCQMC_roberta_base_chinese.model')
    ...
    for name_num in range(len(fidelity_mutants_list)):
        ...
        for data_num in range(len(true_labels)):
            ...
            ori_text_a_list.append(candidate_mutants.loc[data_num, 'ori_text_a'])
            ori_text_b_list.append(candidate_mutants.loc[data_num, 'ori_text_b'])
            text_a_list.append(candidate_mutants.loc[data_num, 'text_a'])
            text_b_list.append(candidate_mutants.loc[data_num, 'text_b'])
        ...
        merge_dt_dict = {'belong': belong_list, 'ori_text_a': ori_text_a_list, 'ori_text_b': ori_text_b_list, 'text_a': text_a_list,
                         'text_b': text_b_list, 'str': str_list, 'word': word_list, 'label': label_list, 'type': mut_list}
