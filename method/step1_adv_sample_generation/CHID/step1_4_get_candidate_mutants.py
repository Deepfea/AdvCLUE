...
from method.step1_adv_sample_generation.CHID.get_sentence import get_sentence
...
def save_candidate_mutants(test_data, mutant, ori_strs, mut_strs, load_rate_path, save_name):
    ...
    for data_num in range(len(test_data)):
        data_id = test_data.loc[data_num, 'data_id']
        ...
        candidate = test_data.loc[data_num, 'candidate']
    ...
    merge_dt_dict = {'data_id': data_id_list, 'tag': tag_list, 'ori_text': ori_text_list, 'text': text_list,
                     'str': str_list, 'word': word_list, 'candidate': candidate_list, 'groundTruth': groundTruth_list,
                     'label': label_list}
