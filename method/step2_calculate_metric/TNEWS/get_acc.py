...
from method.step2_calculate_metric.TNEWS.create_dataset import textDataset
...
def evaluate(dataset, model):
    ...
    # print(total_acc_test / float(len(dataset)))
    return total_acc_test / float(len(dataset))


def cal_ACC(model_name, dataset_name, base_adv_path, base_model_path, base_tokenizer_path):
...
def load_adv_data(dataset_name, base_adv_path):
    ...
    train_data = pd.read_csv(os.path.join(data_path, 'test.csv'))
    return train_data
