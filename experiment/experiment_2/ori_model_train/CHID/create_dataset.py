...
if __name__ == "__main__":
    dataset_path = '/media/usr/external/home/usr/project/project3_data/dataset/CHID'
    train_data = pd.read_csv(os.path.join(dataset_path, 'test.csv'))
    config = Config()

    dataset = ClozeDataset(
        tokenizer=config.TOKENIZER,
        data_id=train_data.data_id.values,
        tag=train_data.tag.values,
        text=train_data.text.values,
        candidate=train_data.candidate.values,
        groundTruth=train_data.groundTruth.values,
