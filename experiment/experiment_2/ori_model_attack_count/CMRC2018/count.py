...
def cal_ASR(model_name, dataset_name, base_adv_path, base_model_path, base_tokenizer_path):
    ...
    acc1_1, acc1_2, results1, results2 = get_acc(model, test_loader)

    result = (acc1_1 + acc1_2) / 2.0
    print(result)
    error_num_list, total_num_list = get_num(test_df, results1, results2)
    print(error_num_list)
    print(total_num_list)


def get_num(df_test, results1, results2):
    mut_name_list = ['word_shuffling', 'character_deleting',
                     ...
                     'nasal_replacement', 'dorsal_rep lacement',
    ...
    for num in range(len(df_test)):
        ...
        if results1[num] == False or results2[num] == False:
...
def get_acc(model, dev_loader):
    ...
    return acc1, acc2, start_list, end_list
