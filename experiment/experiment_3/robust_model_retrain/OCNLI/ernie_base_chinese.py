...
from experiment.experiment_2.ori_model_train.OCNLI.Data import SentencePairDataset
...
def train(model, train_loader, dev_loader, config, model_name):
    ...
    for epoch in range(epochs):
            ...
            if step_cnt % config.eval_steps == 0:
                ...
                if float(dev_accu) > float(best_accu):
                    ...
                    torch.save(model.state_dict(),
                               os.path.join(config.output_model_dir, 'best_OCNLI_' + model_name + '.model'))
        ...
        if float(dev_accu) > float(best_accu):
            best_accu = dev_accu
            torch.save(model.state_dict(), os.path.join(config.output_model_dir, 'best_OCNLI_' + model_name + '.model'))
            print('new epoch saved as the best model {}'.format(epoch))
        model.train()


def train_model(data_path, save_path, load_path, adv_path, model_name):
    config = Config()
