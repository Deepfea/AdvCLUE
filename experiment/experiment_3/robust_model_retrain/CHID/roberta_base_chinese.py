...
from experiment.experiment_2.ori_model_train.CHID.utils import EarlyStopping
...
def train_model(dataset_path, save_path, load_path, adv_path, model_name):
    ...
    adv_train = pd.read_csv(os.path.join(adv_path, 'final_mutants.csv'))[:40000]
    print(len(adv_train))
    adv_candidates = np.load(os.path.join(adv_path, 'final_mutant_candidate.npy'), allow_pickle=True)[:40000]
    print(len(adv_candidates))
    adv_candidate_ids = np.load(os.path.join(adv_path, 'final_mutant_candidate_ids.npy'), allow_pickle=True)[:40000]
    print(len(adv_candidate_ids))

    df_train = pd.concat([df_train, adv_train], ignore_index=True)
    train_candidates = np.concatenate((train_candidates, adv_candidates), axis=0)
    train_candidate_ids = np.concatenate((train_candidate_ids, adv_candidate_ids), axis=0)

    train_dataset = convert_to_features(df_train, train_candidates, train_candidate_ids, config.BERT_PATH)
    train_data_loader = DataLoader(train_dataset, shuffle=False, batch_size=config.batch_size, num_workers=0)
