...
def evaluate_data(dev_loader, model_path):
    ...
    with torch.no_grad():
        for idx, batch_samples in tqdm(enumerate(dev_loader)):
            ...
            label_masks = batch_tags.gt(-1).to('cpu').numpy()
    ...
    for num in range(len(test_output)):
        for tag_num in range(len(test_output[num])):
            output_list.append(test_output[num][tag_num])
            label_list.append(label2id.get(true_tags[num][tag_num]))
...
def get_pic(model_name, dataset_name, base_tokenizer_path, base_adv_path, base_model_path, save_path):
    ...
    adv_data = np.load(os.path.join(data_path, 'final_mutants.npz'), allow_pickle=True)
    x_adv = adv_data["text"]
    ...
    output_arr1, label_list1 = evaluate_data(train_loader, load_model_path)

    load_model_path = os.path.join(base_model_path, 'retrained_model', dataset_name, model_name,
                                   'best_' + dataset_name + '_' + model_name + '.pt')
    output_arr2, label_list2 = evaluate_data(train_loader, load_model_path)
