...
def del_data(model_name, dataset_name, base_adv_path, base_model_path, base_tokenizer_path):
    ...
    for num in tqdm(range(len(flag_list))):
        if flag_list[num] == 0 and temp_num < del_num:
            temp_num += 1
            continue
        belong_list.append(adv_data['belong'][num])
        ori_text_list.append(adv_data['ori_text'][num])
        ori_label_list.append(adv_data['ori_label'][num])
        text_list.append(adv_data['text'][num])
        label_list.append(adv_data['label'][num])
        str_list.append(adv_data['str'][num])
        word_list.append(adv_data['word'][num])
        type_list.append(adv_data['type'][num])
        error_num_list.append(adv_data['error_num'][num])
    np.savez_compressed(os.path.join(base_adv_path, dataset_name, 'filtered_mutants.npz'), belong=belong_list,
                        ori_text=ori_text_list, ori_label=ori_label_list, text=text_list, label=label_list,
                        str=str_list, word=word_list, type=type_list, error_num=error_num_list)

def load_adv_data(dataset_name, base_adv_path):
    data_path = os.path.join(base_adv_path, dataset_name)
    train_data = np.load(os.path.join(data_path, 'selected_mutants.npz'), allow_pickle=True)
    return train_data
