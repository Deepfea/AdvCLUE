...
def load_adv_data(dataset_name, base_adv_path):
    ...
    data = pd.read_csv(os.path.join(data_path, 'filtered_mutants.csv'))
    candidates = np.load(os.path.join(data_path, 'filtered_mutant_candidates.npy'), allow_pickle=True)
    candidate_ids = np.load(os.path.join(data_path, 'filtered_mutant_candidate_ids.npy'), allow_pickle=True)
