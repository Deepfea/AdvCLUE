...
def del_data(model_name, dataset_name, base_adv_path, base_model_path, base_tokenizer_path):
    ...
    for num in range(len(flag_list)):
        ...
        ori_premise_list.append(test_df.loc[num, 'ori_premise'])
        ori_hypothese_list.append(test_df.loc[num, 'ori_hypothese'])
        str_list.append(test_df.loc[num, 'str'])
        word_list.append(test_df.loc[num, 'word'])
        premise_list.append(test_df.loc[num, 'premise'])
        hypothese_list.append(test_df.loc[num, 'hypothese'])
    ...
    merge_dt_dict = {'belong': belong_list, 'ori_premise': ori_premise_list, 'ori_hypothese': ori_hypothese_list,
                     'str': str_list, 'word': word_list,
                     'premise': premise_list, 'hypothese': hypothese_list, 'label': label_list, 'type': type_list,
