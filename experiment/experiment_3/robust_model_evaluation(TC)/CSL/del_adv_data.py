...
def cal_ASR(model_name, dataset_name, base_adv_path, base_model_path, base_tokenizer_path):
    ...
    for num in range(len(flag_list)):
        ...
        ori_abs_list.append(test_df.loc[num, 'ori_abs'])
        ori_keyword_list.append(test_df.loc[num, 'ori_keyword'])
        str_list.append(test_df.loc[num, 'str'])
        word_list.append(test_df.loc[num, 'word'])
        abs_list.append(test_df.loc[num, 'abs'])
        keyword_list.append(test_df.loc[num, 'keyword'])
    ...
    merge_dt_dict = {'belong': belong_list, 'ori_abs': ori_abs_list, 'ori_keyword': ori_keyword_list,
                     'str': str_list, 'word': word_list,
                     'abs': abs_list, 'keyword': keyword_list, 'label': label_list, 'type': type_list,
